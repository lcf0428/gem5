    ...
    ngx_str_t *realm)
{
    ...
    // uint8_t decoded_data[4096];

    // memcpy(decoded_data, start_addr, 4096);

    // printf("write the content to a text file\n");
    // FILE *fp = fopen("output_hex2.txt", "w");
    // if (fp == NULL) return 1;

    // for (int i = 0; i < 4096; i++) {
    //     fprintf(fp, "%02x\n", (unsigned int)decoded_data[i]); // 必须强转
    // }
    // fclose(fp);


    int EXHAUSTED_NUM = 1600;
...
int main(int argc, char **argv)
{
    ...
    ngx_pool_t       *pool = ngx_create_pool(4096 * 4);
    ngx_log_t         log;
